...
UniValue exodus_getproperty(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "  \"issuer\" : \"address\",            (string) the BitcoinZero address of the issuer on record\n"
...
UniValue exodus_getorderbook(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "[                                              (array of JSON objects)\n"
            "  {\n"
            "    \"address\" : \"address\",                         (string) the BitcoinZero address of the trader\n"
...
UniValue exodus_gettradehistoryforaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 3)
        throw runtime_error(
            ...
            "  {\n"
            "    \"txid\" : \"hash\",                               (string) the hex-encoded hash of the transaction of the order\n"
            "    \"sendingaddress\" : \"address\",                  (string) the BitcoinZero address of the trader\n"
            "    \"ismine\" : true|false,                         (boolean) whether the order involes an address in the wallet\n"
            "    \"confirmations\" : nnnnnnnnnn,                  (number) the number of transaction confirmations\n"
            "    \"fee\" : \"n.nnnnnnnn\",                          (string) the transaction fee in bitcoinzeros\n"
            "    \"blocktime\" : nnnnnnnnnn,                      (number) the timestamp of the block that contains the transaction\n"
            ...
            "        \"address\" : \"address\",                         (string) the BitcoinZero address of the other trader\n"
...
UniValue exodus_gettradehistoryforpair(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw runtime_error(
            ...
            "    \"address\" : \"address\",                 (string) the BitcoinZero address of the seller\n"
            "    \"amountsold\" : \"n.nnnnnnnn\",           (string) the number of tokens sold in this trade\n"
            "    \"amountreceived\" : \"n.nnnnnnnn\",       (string) the number of tokens traded in exchange\n"
            "    \"matchingtxid\" : \"hash\",               (string) the hash of the transaction that was matched against\n"
            "    \"matchingaddress\" : \"address\"          (string) the BitcoinZero address of the other party of this trade\n"
            "  },\n"
            "  ...\n"
            "]\n"
            "\nExamples:\n"
...
UniValue exodus_getactivedexsells(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "    \"seller\" : \"address\",               (string) the BitcoinZero address of the seller\n"
            "    \"amountavailable\" : \"n.nnnnnnnn\",   (string) the number of tokens still listed for sale and currently available\n"
            "    \"bitcoinzerodesired\" : \"n.nnnnnnnn\",    (string) the number of bitcoinzeros desired in exchange\n"
            "    \"unitprice\" : \"n.nnnnnnnn\" ,        (string) the unit price (BZX/token)\n"
            ...
            "      {\n"
            "        \"buyer\" : \"address\",                (string) the BitcoinZero address of the buyer\n"
            ...
            "        \"amounttopay\" : \"n.nnnnnnnn\"        (string) the amount in bitcoinzeros needed finalize the trade\n"
            "      },\n"
            "      ...\n"
            "    ]\n"
            "  },\n"
    ...
    for (OfferMap::iterator it = my_offers.begin(); it != my_offers.end(); ++it) {
        ...
        int64_t sellBitcoinDesired = selloffer.getBZXDesiredOriginal(); //badly named - "Original" implies off the wire, but is amended amount
        ...
        responseObj.push_back(Pair("bitcoinzerodesired", FormatDivisibleMP(bitcoinDesired)));
        ...
        for (AcceptMap::const_iterator ait = my_accepts.begin(); ait != my_accepts.end(); ++ait) {
            ...
            if (accept.getHash() == selloffer.getHash()) {
                ...
                int64_t amountToPayInBTC = calculateDesiredBTC(accept.getOfferAmountOriginal(), accept.getBZXDesiredOriginal(), amountAccepted);
...
UniValue exodus_gettransaction(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "  \"sendingaddress\" : \"address\",     (string) the BitcoinZero address of the sender\n"
            "  \"referenceaddress\" : \"address\",   (string) a BitcoinZero address used as reference (if any)\n"
...
UniValue exodus_getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"bitcoinzerocoreversion\" : \"x.x.x\",        (string) BitcoinZero Core version\n"
    ...
    // provide the Exodus and BitcoinZero version
    infoResponse.push_back(Pair("exodusversion_int", EXODUS_VERSION));
    infoResponse.push_back(Pair("exodusversion", ExodusVersion()));
    infoResponse.push_back(Pair("bitcoinzerocoreversion", BitcoinZeroCoreVersion()));
...
UniValue exodus_getsto(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "  \"recipients\": [                 (array of JSON objects) a list of recipients\n"
            "    {\n"
            "      \"address\" : \"address\",          (string) the BitcoinZero address of the recipient\n"
