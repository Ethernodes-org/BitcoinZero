...
CPubKey CWallet::GenerateNewKey(uint32_t nChange) {
    ...
    uint32_t nIndex = (regTest || testNet) ? BIP44_TEST_INDEX : BIP44_BITCOINZERO_INDEX;
    ...
    if (!hdChain.masterKeyID.IsNull()) {
        ...
        CExtKey coinTypeKey;           //key at m/44'/<1/136>' (Testnet or BitcoinZero Coin Type respectively, according to SLIP-0044)
        ...
        const CCoinControl *coinControl) const
{
    ...
    if (!MoneyRange(required)) {
        throw std::invalid_argument("Request to spend more than 21 MLN bitcoinzeros.\n");
    }
            ...
            while (true) {
                ...
                if (!SelectCoins(vAvailableCoins, nValueToSelect, setCoins, nValueIn, coinControl)) {
                    if (nCoinType == ONLY_NOT1000IFMN) {
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal 1000 BZX.");
                    } else if (nCoinType == ONLY_NONDENOMINATED_NOT1000IFMN) {
                        strFailReason = _("Unable to locate enough PrivateSend non-denominated funds for this transaction that are not equal 1000 BZX.");
                    } else if (nCoinType == ONLY_DENOMINATED) {
...
bool CWallet::CreateSigmaMintModel(
    ...
    for(const std::pair<sigma::CoinDenomination, int>& denominationPair: denominationPairs) {
        ...
        if(coinCount < 0) {
            throw runtime_error("Coin count negative (\"bitcoinzeroaddress\")\n");
        }
...
bool CWallet::CreateZerocoinMintModelV2(
    ...
    BOOST_FOREACH(denominationPair, denominationPairs){
        ...
        switch(denominationValue){
            ...
            default:
                throw runtime_error(
                    "mintzerocoin <amount>(1,10,100,250,500) (\"bitcoinzeroaddress\")\n");
        }
...
bool CWallet::CreateZerocoinToSigmaRemintModel(string &stringError, int version, libzerocoin::CoinDenomination denomination, CWalletTx *wtx) {
    ...
    if (!params.IsRegtest() && !bznodeSync.IsBlockchainSynced()) {
